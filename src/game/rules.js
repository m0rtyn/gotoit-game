  ...
  nextDay: {
    onTick: function(state) {
      ...
      data.candidates.resumes.forEach(candidate => {
        if (candidate.stats.design > data.max_candidates_stat)
          data.max_candidates_stat = candidate.stats.design;
        if (candidate.stats.program > data.max_candidates_stat)
          data.max_candidates_stat = candidate.stats.program;
        if (candidate.stats.manage > data.max_candidates_stat)
          data.max_candidates_stat = candidate.stats.manage;
      });
      //offered_projects
      data.offered_projects.forEach(project => {
        if (project.estimate.design > data.max_stats_projects_offered)
          data.max_stats_projects_offered = project.estimate.design;
        if (project.estimate.program > data.max_stats_projects_offered)
          data.max_stats_projects_offered = project.estimate.program;
        if (project.estimate.manage > data.max_stats_projects_offered)
          data.max_stats_projects_offered = project.estimate.manage;
      });
      ...
      data.on_tick_effects = _.filter(data.on_tick_effects, effect => {
        ...
        return (
          public_relations[effect.type].long >
          data.date.tick - effect.start_tick
        );
      });

      _.each(data.on_tick_effects, effect => {
        public_relations[effect.type].onTickByDelta(
          data,
          data.date.tick - effect.start_tick,
          effect.click_count
        );
