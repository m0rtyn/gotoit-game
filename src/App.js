...
class App extends Component {
  ...
  UNSAFE_componentWillMount() {
    ...
    let loaded_app_state = JSON.parse(
      localStorage.getItem(game_name + '_app_state')
    );
    ...
    if (loaded_app_state) {
      ...
      _.each(loaded_app_state.data.workers, (worker, id) => {
        loaded_app_state.data.workers[id] = _.create(
      ...
      _.each(loaded_app_state.data.candidates.resumes, (worker, id) => {
        loaded_app_state.data.candidates.resumes[id] = _.create(
          WorkerModel.prototype,
          worker
        );
      });
      ...
      _.each(loaded_app_state.data.candidates.agency, (worker, id) => {
        loaded_app_state.data.candidates.agency[id] = _.create(
          WorkerModel.prototype,
          worker
        );
      });
      ...
      _.each(loaded_app_state.data.projects, (project, id) => {
        loaded_app_state.data.projects[id] = _.create(
      ...
      _.each(loaded_app_state.data.offered_projects, (project, id) => {
        loaded_app_state.data.offered_projects[id] = _.create(
          ProjectModel.prototype,
          project
        );
      });
      ...
      _.each(loaded_app_state.data.projects_end_reports, (project, id) => {
        loaded_app_state.data.projects_end_reports[id] = _.create(
          ProjectModel.prototype,
          project
        );
      });
      ...
      console.log(
        'App ' + game_name + ' componentDidMount with state',
        loaded_app_state
      );
  ...
  playGame() {
    ...
    this.timerID = setInterval(
      () => this.tick(true),
      Math.floor(
        this.state.data.game_speed / this.state.data.game_speed_multiplier
      )
    );
  ...
  newGame() {
    ...
    if (
      !window.confirm(
        'Are you ready to start a new game? Your progress will be lost.'
      )
    )
      return false;
  ...
  modifyRelationPure(
    ...
    let put = (worker_id, project_id) => {
      ...
      if (!(project_id in data.relations[worker_id]))
      ...
      if (role) {
        ...
        data.relations[worker_id][project_id] = _.clone(
          data.workers_roles[worker_id]
  ...
  getRole(worker_id, role) {
    if (
      worker_id in this.state.data.workers_roles &&
      role in this.state.data.workers_roles[worker_id]
    )
  ...
  changeRole(worker_id, role, value) {
    const data = this.state.data;
    if (!(worker_id in data.workers_roles))
      data.workers_roles[worker_id] = JSON.parse(JSON.stringify(skills_true));
  ...
  riseEmployer(worker_id) {
    ...
    addMessage(
      'You raised salary to ' + worker.name + '!',
  ...
  startMeeting(meeting_name, selected_workers) {
    ...
    addMessage(
      'Start ' + meeting_conf.name + ' meeting',
      { timeOut: 5000, extendedTimeOut: 2000 },
      'info'
    );
    this.setState({ data: data });
  ...
  startOffered(id) {
    ...
    data.helpers.addTimelineEvent(
      'deadline',
      'Deadline',
      project,
      project.deadline_max / 24
    );
    console.log('start');
  }
  ...
  pauseProject(id) {
    ...
    console.log('pause');
    let newTimelineEvents = this.state.data.timelineEvents.filter(
      i => i.object.name !== project.name
    );
  ...
  unpauseProject(id) {
    ...
    this.state.data.helpers.addTimelineEvent(
      'deadline',
      'Deadline',
      project,
      project.deadline / 24
  ...
  finishProject(id) {
    ...
    let platform_kind_top_handler = all_top_handler
      .filter('platform', project.platform)
      .filter('kind', project.kind);
    ...
    if (project.type === 'own') {
      ...
      project.reward =
        ...
        _.sum(
          _.values(
            bulkStyler.projectPlatform(
              bulkStyler.projectKind(project.done, project.kind),
              project.platform
            )
          )
        );
  ...
  projectReporting(project_id, stage) {
    ...
    if (['fail', 'close'].includes(stage) && project.penalty !== 0) {
      ...
      let newTimelineEvents = data.timelineEvents.filter(
        i => i.object.name !== project.name
      );
      data.timelineEvents = newTimelineEvents;
    }
  ...
  projectArchiving() {
    ...
    data.statistics.projects_done.buffer = data.projects_archive_reports.filter(
      project => {
    ...
    if (
      ...
      !data.attainments.includes('FirstTraining')
    ) {
  ...
  changeTechnology(technology, project_id, value) {
    const data = this.state.data;
    if (!(project_id in data.projects_technologies))
  ...
  chargeMoney(quantity, silent = false) {
    ...
    if (!silent)
      addAction(
        'Charge from your wallet: ' + quantity + '$',
  ...
  calcSalary(worker, current_tick) {
    ...
    let last_month_worked_days = Math.floor(
      (current_tick - worker.facts.prev_salary_payment_tick) / 24
    );

    // if the employee worked full month he get fixed monthly salary,
    // else salary calculating in depending on the days worked
    return last_month_worked_days < 30
      ? Math.floor(last_month_worked_days * daily_salary)
      : worker.getSalary();
  }
  ...
  nextDay() {
    ...
    if (time.hour === 0) {
      ...
      data.workers.forEach(worker => {
        ...
        if (!worker.is_player) {
          let dissatisfaction = Math.floor(
            (10000 - Math.pow(worker.calcEfficiency() + 50, 2)) / 30
          );
    ...
    if (time.date !== 1 && game_date.getDate() === 1) {
      ...
      data.taken_loans.forEach(loan => {
        this.chargeMoney(
          Math.floor((loan.money * (1 + loan.interest / 100)) / loan.time)
        );
        loan.timer--;
  ...
  rollTurn() {
    ...
    let probability =
      Math.min(10, 1 + data.workers.length * projects_done * 0.1) / 24;
    if (
      data.offered_projects.length < 5 &&
      _.random(0.0, 100.0) < probability
    ) {
      this.pushNewProject();
    }
    ...
    if (
      Math.floor(
        _.random(
          1,
          24 * (50 - Math.max(spike, Math.min(25, projects_done * 0.2)))
        )
      ) === 1 &&
    ...
    if (
      Math.floor(_.random(1, (24 * 7 * 12) / (1 + projects_done * 0.1))) ===
        1 &&
      data.candidates.resumes.length < 5
    ) {
  ...
  pushNewProject() {
    const data = this.state.data;
    let quality = Math.ceil(
      _.random(1, current_tick / (24 * 30) + projects_done * 0.1)
    );
  ...
  work() {
    ...
    _.shuffle(data.workers).forEach(worker => {
      ...
      if (!worker.is_player) {
        ...
        if (
          _.random(1, (1 + parseFloat(worker.getOverrate())).toFixed(0)) === 1
        ) {
  ...
  work_on_project(worker, project) {
    ...
    if (
      ...
      this.getTechnology(project.id, 'refactoring')
    ) {
      if (
        project.complexity < project.tasksQuantity() + project.bugsQuantity() &&
        _.random(
          1,
          (worker.effects['review'] > 0 ? 2 : 0.5) * project.complexity
        ) >
          _.random(
            [0, 0.1, 1, 2, 3, 4][project.size] * Math.sqrt(project.complexity),
            Math.sqrt(project.planedTasksQuantity())
          )
      ) {
        let refactoring = Math.min(
          project.complexity,
          worker.getRareSideResource()
        );
    ...
    if (
      !skip_work &&
      this.getTechnology(project.id, 'pair') &&
      !project.supporter
    ) {
