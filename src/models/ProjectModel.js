...
class ProjectModel {
  ...
  applyWork(
    ...
    Object.keys(work).forEach(stat => {
      if (this.needs(stat) > 0 && work[stat] > 0) {
        var support =
          this.supporter && this.supporter.id !== worker.id
            ? this.supporter.stats[stat]
            : 0;
        let all_work =
          _.random(
            1,
            work[stat] + support + (rad ? worker.getSideResource() : 0)
          ) + this.stored_wisdom[stat];
        let complexity_penalty = Math.max(
          0,
          Math.floor(
            Math.sqrt(
              Math.max(0, this.complexity - _.random(0, this.bugs[stat]))
            )
          ) -
            Math.pow(this.iteration, 2) +
            1
        );
        ...
        if (bugs > 0) {
          ...
          if (prevented) {
            if (tasks !== 0) {
              animation.addBubbleAnimation(
                focus_on,
                tasks,
                worker.id,
                project.id + project_bars[`${focus_on}_completed`].id
              );
            }
            if (bugs !== 0) {
              console.log('bug!');
              animation.addBubbleAnimation(
                ...
                bugs,
                worker.id,
                project.id + project_bars[`${focus_on}_bugs`].id,
                true
              );
            }
        ...
        let boost_from_character =
          worker.character.name === 'Gifted'
            ? 2
            : worker.character.name === 'Modest'
              ? 0.5
              : 1;
  ...
  fix() {
    ...
    _.each(this.bugs, (count, skill) => {
      if (this.estimate[skill] === Number.POSITIVE_INFINITY)
        this.estimate[skill] = 0;
  ...
  static genStat(quality, size = 1) {
    ...
    let d =
      _.random(0, quality) *
      (0 + _.random(1, Math.floor(Math.sqrt(projects_done * 2))));
    let g =
      _.random(0, quality) *
      (0 + _.random(1, Math.floor(Math.sqrt(projects_generated * 0.1))));
    let r = _.random(1, 10);
