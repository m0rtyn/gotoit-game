...
class WorkerModel {
  constructor(
    ...
    this.id = is_player
      ? 'player'
      : _.uniqueId('worker') + '_' + _.random(100000000, 999999999);
    this.name = name;
    this.gender = gender;
    this.stats = stats;
    ...
    this.salary_coefficient =
      this.character.name === 'Workaholic'
        ? -15
        : this.character.name === 'Modest'
          ? 20
          : 0;
    this.thirst_to_knowledge_coefficient =
      this.character.name === 'Gifted'
        ? 0.75
        : this.character.name === 'Wonk'
          ? 1.25
          : 1;
  ...
  getSalary() {
    if (this.is_player) {
      ...
      return Math.floor(
        (this.statsSum() + _.max(_.values(this.stats))) *
          (1 + this.getOverrate() / 100) *
          160
      );
    }
  }
  ...
  isWorkingTime(time, micromanagement, motivation, office_things) {
    ...
    let is_working_time =
      time.hour >=
        9 +
          (office_things.coffeemaker ? -1 : 0) +
          mod +
          character_working_mod &&
      time.hour <=
        17 +
          (this.fed_ticker > 1 ? 2 : 0) +
          (this.effects['status'] > 0 ? 3 : 0) +
          mod &&
  ...
  workloadPenalty() {
    const task_preferred =
      Math.ceil((current_tick - this.facts.tick_hired) / 24) * 3;
        ...
        Math.floor(
          20 *
            (1 -
              (200 + task_preferred) /
                (200 +
                  (this.facts.tasks_done - this.facts.training_tasks_done)))
        ),
        20
      ),
      -20
  ...
  difficultyPenalty() {
    let difficulty_ratio =
      (200 + this.facts.bugs_passed * 2) / (200 + this.facts.tasks_done);
    let thirst_for_difficulty =
      (100 + _.max(_.values(this.stats))) / (100 + this.statsSum() / 4);
    // console.log(difficulty_ratio, thirst_for_difficulty);
    const difficulty_stream =
      20 * (1 - difficulty_ratio / thirst_for_difficulty);
  ...
  educationPenalty() {
    let knowledge_ratio =
      (200 + this.facts.training_tasks_done * 4) /
      (200 + this.facts.tasks_done);
    let thirst_for_knowledge =
      ((100 + this.statsSum() / 4) / (100 + _.max(_.values(this.stats)))) *
      this.thirst_to_knowledge_coefficient;
  ...
  collectivePenalty() {
    ...
    const collective =
      20 * (1 - (10 + collective_avg) / (10 + this.statsSum()));
  ...
  getOverrate() {
    return (
      (1 + this.standing / (24 * 7 * (59.524 + this.salary_coefficient))) *
        100 -
      100
    ).toFixed(2);
  }
  ...
  calcEfficiencyReal() {
    ...
    return this.get_monthly_salary === false
      ? Math.ceil(happiness / 2)
      : Math.ceil(happiness);
  ...
  getStatsData(stat) {
    return (
      ...
      (this.items[stat].flat === true
        ? ' +' + workers_bonus_items[stat].flat.bonus + ' '
        : '') +
      (this.items[stat].percent === true
        ? ' +' + workers_bonus_items[stat].percent.bonus + '% '
        : '')
    );
  }
  ...
  gotoMeeting(meeting) {
    ...
    if (
      this.effects[meeting.meeting_type] >
      meetings[meeting.meeting_type].max_bonus
    )
      return false;

    this.effects[meeting.meeting_type] += Math.floor(
      meetings[meeting.meeting_type].max_bonus /
        meetings[meeting.meeting_type].deadline
    );
  }
  ...
  static generateAgency(agency_state) {
    ...
    let stats = _.mapValues(skills, (value, skill) => {
      let stat = _.random(
        agency_state.min_stats[skill],
        agency_state.max_stats[skill]
      );
    ...
    worker.standing = Math.floor(
      _.random(agency_state.min_salary, agency_state.max_salary) * 26.888
    );
    //console.log(worker);
    return worker;
  }
