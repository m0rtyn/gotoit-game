...
class HiringAgency extends Component {
  ...
  calcCost() {
    ...
    let pike_factor1 = Math.floor(
      Math.pow(_.max(_.values(s.min_stats)) + _.max(_.values(s.max_stats)), 2.5)
    );
    let pike_factor2 = Math.floor(
      Math.pow(_.max(_.values(s.max_stats)) - _.min(_.values(s.max_stats)), 2)
    );
    ...
    skills_names.forEach(skill => {
      sum_control_factor += Math.pow(
        s.max_stats[skill] - s.min_stats[skill],
        2
      );
    });
    ...
    let salary_control_factor =
      Math.pow(s.max_salary - s.min_salary, 2.5) /
    ...
    return (
      ...
      Math.floor(
        ((1 + s.deal_counter / 10) *
          (1000 + min_sum_factor + max_sum_factor + pike_factor1)) /
          (0.0006 *
            (500 +
              min_salary_factor +
              max_salary_factor +
              sum_control_factor +
              salary_control_factor +
              pike_factor2))
      )
    );
  }
  ...
  render() {
    ...
    const search_button = (
      <button className="btn btn-info hidden">Hiring Agency</button>
    );
      ...
      <Portal ref="agency" closeOnEsc openByClickOn={search_button}>
        <TeamDialog>
          <div className="text-center">
            ...
            <p>
              Choose search criteria. Leave our personnel officers leeway to
