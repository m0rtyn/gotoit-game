...
class Loans extends Component {
  ...
  getEarlyCost(loan) {
    return Math.floor(
      ((loan.money * (1 + loan.interest / 100)) / loan.time) * loan.timer
    );
  }
  ...
  calcCreditScore() {
    ...
    const inertia = 500;
    let good_factors =
      Math.sqrt(data.old_loans.length) * 100 +
      Math.sqrt(data.early_payed_loans) * 1;
      ...
      <div className="text-center">
        ...
        <div className="card">
          {data.taken_loans.map((loan, i) => {
            let paid_percent = Math.ceil(
              ((loan.time - loan.timer) / loan.time) * 100
            );
                ...
                <div className="">
                  ...
                  <span className={paid_percent ? '' : ' hidden'}>
                    <button
                      ...
                      onClick={() => {
                        if (this.getEarlyCost(loan) <= data.money)
