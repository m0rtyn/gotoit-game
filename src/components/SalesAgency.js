...
import { project_sizes, roles, skills, skills_names } from '../game/knowledge';
...
class SalesAgency extends Component {
  ...
  calcCost() {
    ...
    let pike_factor1 = Math.floor(
      (_.max(_.values(s.min_stats)) + _.max(_.values(s.max_stats))) / 2
    );
    let pike_factor2 = Math.floor(
      (_.max(_.values(s.max_stats)) - _.min(_.values(s.max_stats))) / 2
    );
  ...
  render() {
    ...
    const search_button = (
      <button className="btn btn-info hidden">Sales Agency</button>
    );
      ...
      <Portal ref="agency" closeOnEsc openByClickOn={search_button}>
        <TeamDialog>
          <div className="text-center">
            ...
            <p>
              Choose search criteria. Leave our sales manager leeway to reduce
